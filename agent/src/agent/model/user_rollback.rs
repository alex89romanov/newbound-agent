...
// user_rollback: POST /user_rollback - revert the user-facing pointer
// to last_good (one step, deliberately not a stack). The manual twin
// of the watchdog's automatic rollback.
...
let url = format!("http://127.0.0.1:{}/user_rollback", prop("MODEL_SERVICE_PORT", "8077"));
...
match resp {
    ...
    Err(ureq::Error::Status(_code, r)) => match r.into_string() {
        Ok(t) => match DataObject::try_from_string(&t) {
            Ok(d) => d,
            Err(_) => err("user rollback refused".to_string()),
        },
        Err(e) => err(format!("service read failed: {}", e)),
    },
