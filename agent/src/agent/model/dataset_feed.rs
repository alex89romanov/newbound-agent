...
pub fn execute(o: DataObject) -> DataObject {
    use std::panic;
    for p in ["name", "kind", "lines", "lineage", "provenance", "holdout_every"] {
        if !o.has(p) {
    ...
    let ax = panic::catch_unwind(panic::AssertUnwindSafe(|| {
        ...
        let arg_1: String = o.get_string("kind");
        let arg_2: String = o.get_string("lines");
        let arg_3: String = o.get_string("lineage");
        let arg_4: String = o.get_string("provenance");
        let arg_5: i64 = o.get_int("holdout_every");
        dataset_feed(arg_0, arg_1, arg_2, arg_3, arg_4, arg_5)
    }));
    match ax {
        Ok(ax) => {
            let mut result_obj = DataObject::new();
