...
pub fn execute(o: DataObject) -> DataObject {
    use std::panic;
    for p in ["name", "dataset", "base", "targets", "rank", "steps"] {
        if !o.has(p) {
    ...
    let ax = panic::catch_unwind(panic::AssertUnwindSafe(|| {
        ...
        let arg_3: String = o.get_string("targets");
        let arg_4: i64 = o.get_int("rank");
        let arg_5: i64 = o.get_int("steps");
        adapter_derive(arg_0, arg_1, arg_2, arg_3, arg_4, arg_5)
    }));
    match ax {
        Ok(ax) => {
            let mut result_obj = DataObject::new();
